...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (groqApiKey) {
      return await askGroq({ res, apiKey: groqApiKey, messages, system, maxTokens });
    }

    if (geminiApiKey) {
      return await askGemini({ res, apiKey: geminiApiKey, messages, system, maxTokens });
    }

    if (anthropicApiKey) {
      return await askAnthropic({ res, apiKey: anthropicApiKey, messages, system, maxTokens });
    }

    return res.status(200).json({ reply: createPlaceholderReply(messages, system) });
  } catch (error) {
    return res.status(500).json({ error: error.message || "AI function failed." });
  }
};

async function askGroq({ res, apiKey, messages, system, maxTokens }) {
  ...
  return res.status(200).json({ reply: reply || createPlaceholderReply(messages, system), provider: "groq" });
}

async function askGemini({ res, apiKey, messages, system, maxTokens }) {
  ...
  return res.status(200).json({ reply: reply || createPlaceholderReply(messages, system), provider: "gemini" });
}

async function askAnthropic({ res, apiKey, messages, system, maxTokens }) {
  ...
  return res.status(200).json({ reply: reply || createPlaceholderReply(messages, system), provider: "anthropic" });
}
...
function createPlaceholderReply(messages, system) {
  ...
  if (lowerSystem.includes("main ai assistant")) {
    if (lowerMessage.includes("calendar") || lowerMessage.includes("appointment") || lowerMessage.includes("therapy")) {
      return 'Done — I added it to your calendar and Friday timeline.\n\n```action\n{"type":"add_calendar","data":{"date":"29","title":"Therapy Session","type":"appointment"}}\n```\n```action\n{"type":"add_schedule","data":{"date":"29","time":"1:00 PM","event":"Therapy Session","type":"routine"}}\n```';
    }
