...
const BUDGET_AI = `You are the user's personal financial coach. Be warm, specific, concise (2-4 sentences). Reference actual numbers when shared. Always end with a question or actionable next step.`;
...
- add_budget: {"category": "income|expense|bills|debt|saving", "name": "...", "planned": 0, "actual": 0}
...
export default function App() {
  ...
  const handleSignup = () => {
    ...
    const newUser = { id: `u_${Date.now()}`, email, displayName, password }; // password stored locally for demo
    users[email] = newUser;
  ...
  useEffect(() => {
    if (!user) return;
    setScheduleItems(loadLocal(user.id, "schedule", defaultSchedule));
    setCalendarEvents(loadLocal(user.id, "calendar", []));
    setBudget(loadLocal(user.id, "budget", defaultBudget));
    setNotes(loadLocal(user.id, "notes", defaultNotes));
    setObjections(loadLocal(user.id, "objections", defaultObjections));
    setInboundScript(loadLocal(user.id, "inboundScript", defaultInboundScript));
    setOutboundScript(loadLocal(user.id, "outboundScript", defaultOutboundScript));
    setMedsList(loadLocal(user.id, "meds", []));
    setStars(loadLocal(user.id, "stars", 0));
    setAppointments(loadLocal(user.id, "appointments", 0));
    setCallStreak(loadLocal(user.id, "callStreak", 0));
    setCallLog(loadLocal(user.id, "callLog", []));
    setWaterGlasses(loadLocal(user.id, "water", 0));
    setReminders(loadLocal(user.id, "reminders", []));
    setNotifEnabled(loadLocal(user.id, "notifEnabled", false));
  ...
  // ─── SAVE on changes ───────────────────────────
  useEffect(() => { if (user) saveLocal(user.id, "schedule", scheduleItems); }, [scheduleItems, user]);
  useEffect(() => { if (user) saveLocal(user.id, "calendar", calendarEvents); }, [calendarEvents, user]);
  useEffect(() => { if (user) saveLocal(user.id, "budget", budget); }, [budget, user]);
  useEffect(() => { if (user) saveLocal(user.id, "notes", notes); }, [notes, user]);
  useEffect(() => { if (user) saveLocal(user.id, "objections", objections); }, [objections, user]);
  useEffect(() => { if (user) saveLocal(user.id, "inboundScript", inboundScript); }, [inboundScript, user]);
  useEffect(() => { if (user) saveLocal(user.id, "outboundScript", outboundScript); }, [outboundScript, user]);
  useEffect(() => { if (user) saveLocal(user.id, "meds", medsList); }, [medsList, user]);
  useEffect(() => { if (user) saveLocal(user.id, "stars", stars); }, [stars, user]);
  useEffect(() => { if (user) saveLocal(user.id, "appointments", appointments); }, [appointments, user]);
  useEffect(() => { if (user) saveLocal(user.id, "callStreak", callStreak); }, [callStreak, user]);
  useEffect(() => { if (user) saveLocal(user.id, "callLog", callLog); }, [callLog, user]);
  useEffect(() => { if (user) saveLocal(user.id, "water", waterGlasses); }, [waterGlasses, user]);
  useEffect(() => { if (user) saveLocal(user.id, "reminders", reminders); }, [reminders, user]);
  useEffect(() => { if (user) saveLocal(user.id, "notifEnabled", notifEnabled); }, [notifEnabled, user]);
  ...
  const handleSectionAISubmit = async (section) => {
    ...
    if (section === "budget") {
      const totals = ["income","expense","bills","debt","saving"].reduce((acc, k) => {
        acc[k] = budget[k].reduce((s, i) => s + (parseFloat(i.actual) || 0), 0);
        return acc;
      }, {});
      context = `\n\nCurrent budget: Income $${totals.income}, Expenses $${totals.expense}, Bills $${totals.bills}, Debt $${totals.debt}, Saving $${totals.saving}.`;
    ...
    try {
      ...
      setSectionAIChats(p => ({ ...p, [section]: [...updatedChat, { role: "assistant", content: "Oops, had a hiccup! Try again 💕" }] }));
    }
  ...
  const handleMasterAISubmit = async () => {
    ...
    try {
      ...
      setMasterChat([...updatedChat, { role: "assistant", content: "Oops, had a hiccup!" }]);
    }
  ...
  const executeAction = (action) => {
    ...
    switch (action.type) {
      ...
      case "add_schedule":
        setScheduleItems(p => [...p, { id: Date.now(), time: action.data.time, event: action.data.event, type: action.data.type || "work" }]);
        ...
        <div className="w-full max-w-sm rounded-[2rem] p-7 shadow-2xl backdrop-blur-xl border-2 border-white relative overflow-hidden" style={{background: "rgba(255,255,255,0.85)"}}>
          ...
          <div className="relative">
            ...
            <div className="space-y-3">
              ...
              {authError && <div className="text-xs font-medium px-3 py-2 rounded-xl" style={{background: palette.pink, color: palette.pinkText}}>⚠️ {authError}</div>}
            ...
            <div className="space-y-4">
              {scheduleItems.map((item, index) => (
                <div key={item.id} className="flex relative items-center">
                  {index !== scheduleItems.length - 1 && <div className="absolute left-[11px] top-6 bottom-[-16px] w-[2px]" style={{background: `linear-gradient(180deg, ${palette.lavender}, transparent)`}}></div>}
  ...
  const renderBudget = () => {
    ...
    const pieData = [
      { label: "Expenses", value: totals.expenseActual, color: palette.peachDeep, tc: palette.peachText },
      { label: "Bills", value: totals.billsActual, color: palette.lavenderDeep, tc: palette.lavenderText },
      { label: "Debt", value: totals.debtActual, color: palette.pinkDeep, tc: palette.pinkText },
      { label: "Saving", value: totals.savingActual, color: palette.skyDeep, tc: palette.skyText },
    ].filter(d => d.value > 0);
    const totalOut = totals.expenseActual + totals.billsActual + totals.debtActual + totals.savingActual;
    const remaining = totals.incomeActual - totalOut;
    const tableConfigs = [
          ...
          <table className="w-full text-left text-xs">
            <thead style={{background: `${color}30`}}>
              <tr style={{color: tc}}>
                ...
                <th className="p-2 font-bold">PLAN</th>
                <th className="p-2 font-bold">ACT</th>
            ...
            <tbody>
              ...
              <tr style={{background: `${color}20`}}>
                ...
                <td className="p-1.5"><input type="number" placeholder="$0" value={newBudgetItems[key].planned} onChange={e => setNewBudgetItems(p => ({...p, [key]: {...p[key], planned: e.target.value}}))} className="w-full p-1.5 text-xs rounded-md" style={{background: "white", border: `1px solid ${color}`}}/></td>
                <td className="p-1.5"><input type="number" placeholder="$0" value={newBudgetItems[key].actual} onChange={e => setNewBudgetItems(p => ({...p, [key]: {...p[key], actual: e.target.value}}))} className="w-full p-1.5 text-xs rounded-md" style={{background: "white", border: `1px solid ${color}`}}/></td>
                {showPaid && <td></td>}
      ...
      <div className="animate-in fade-in duration-500 space-y-5">
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-3xl p-4 border-2 border-white shadow-md" style={{background: `linear-gradient(135deg, ${palette.mint}90, ${palette.sky}60)`}}>
            <div className="text-xs font-bold uppercase tracking-wider mb-1" style={{color: palette.mintText, opacity: 0.7}}>INCOME</div>
            <div className="text-3xl font-extrabold" style={{color: palette.mintText, fontFamily: "'Fraunces', serif"}}>${totals.incomeActual}</div>
          </div>
          <div className="rounded-3xl p-4 border-2 border-white shadow-md" style={{background: remaining >= 0 ? `linear-gradient(135deg, ${palette.lavender}90, ${palette.sky}60)` : `linear-gradient(135deg, ${palette.pink}90, ${palette.peach}60)`}}>
            <div className="text-xs font-bold uppercase tracking-wider mb-1" style={{color: remaining >= 0 ? palette.lavenderText : palette.pinkText, opacity: 0.7}}>{remaining >= 0 ? "LEFT" : "OVER"}</div>
        ...
        <div className="rounded-3xl p-5 shadow-md border-2 border-white" style={{background: "rgba(255,255,255,0.85)"}}>
          ...
          {totalOut === 0 ? (
            <div className="text-center py-8"><div className="text-4xl mb-2">✨</div><p className="text-xs font-medium" style={{color: palette.lavenderText}}>Add expenses to see your breakdown!</p></div>
          ) : (
            <div className="flex items-center gap-5 flex-col sm:flex-row">
              <div className="shrink-0"><PieChart data={pieData} size={180}/></div>
              <div className="flex-grow space-y-2 w-full">
                {pieData.map((d, idx) => {
                  const pct = Math.round((d.value / totalOut) * 100);
                  return (
